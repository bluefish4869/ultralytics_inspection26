...
def generate_data_json(image_paths, output_dir, file_name_base, image_format):
    ...
    try:
        ...
        # 提示如何使用
        print("\n💡 使用方法:")
        print("  1. 在Label Studio中创建新项目")
        print("  2. 导入数据时选择 'JSON' 格式")
        print(f"  3. 上传 {json_path} 文件")
        print(f"  4. 图片文件在 {output_dir} 目录下")
        print("  5. 注意: 确保Label Studio有权限访问这些图片的绝对路径")
...
def main():
    ...
    parser.add_argument("-o", "--output", default=None, help="输出文件夹路径 (默认: ./runs/时间戳)")
    parser.add_argument("-i", "--interval", type=int, default=1, help="帧提取间隔，1为每帧都提取 (默认: 1)")
    parser.add_argument("-f", "--format", choices=["jpg", "png"], default="jpg", help="输出图片格式 (默认: jpg)")
    parser.add_argument("-q", "--quality", type=int, default=95, help="图片质量，仅对jpg有效，范围1-100 (默认: 95)")
