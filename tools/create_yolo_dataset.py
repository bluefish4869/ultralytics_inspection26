...
class YOLOSubsetCreator:
    ...
    def __init__(
        self, dataset_root: str, subset_name: str | None = None, output_dir: str | None = None, random_seed: int = 42
    ):
        """初始化.
    ...
    def filter_by_classes(
        self, data: dict[str, list[int]], target_classes: list[int] | None = None, include_all: bool = True
    ) -> dict[str, list[int]]:
        """按类别筛选.
    ...
    def sample_data(
        self, data: dict[str, list[int]], max_count: int | None = None, random_sample: bool = False
    ) -> dict[str, list[int]]:
        """采样数据.
    ...
    def split_dataset(
        self, data: dict[str, list[int]], val_ratio: float = 0.2, random_split: bool = True
    ...
    def create_list_file(self, data: dict[str, list[int]], output_file: str):
        """创建列表文件.
    ...
    def create_data_yaml(self, train_file: str, val_file: str, output_file: str, nc: int, names: list[str]):
        """创建 data.yaml 配置文件.
...
"""
    ...
    def create_summary(self, all_data: dict, train_data: dict, val_data: dict, output_file: str):
        """创建摘要报告."""
        output_path = Path(output_file)
        output_path.parent.mkdir(parents=True, exist_ok=True)
...
使用示例:
  ...
  python create_yolo_subset_simple.py \\
    ...
    # 必需参数
    parser.add_argument("--dataset", "-d", required=True, help="数据集根目录 (包含 images/ 和 labels/ 子目录)")
    parser.add_argument("--name", "-n", default=None, help="子集名称 (默认: 自动生成时间戳)")

    # 筛选参数
    parser.add_argument("--classes", "-c", type=int, nargs="+", help="要筛选的类别ID (如: 0 1 2)")
    parser.add_argument("--include_all", action="store_true", help="必须包含所有指定类别 (默认: 包含任意)")
    parser.add_argument("--max_count", "-m", type=int, help="最大图片数量")
    parser.add_argument("--random", action="store_true", help="随机采样 (默认: 顺序采样)")

    # 分割参数
    parser.add_argument("--val_ratio", "-r", type=float, default=0.2, help="验证集比例 (默认: 0.2)")
    parser.add_argument("--no_split", action="store_true", help="不分割数据集，只生成训练集")
    parser.add_argument("--no_random_split", action="store_true", help="不随机分割 (默认: 随机分割)")

    # 类别名称
    parser.add_argument("--nc", type=int, help="类别数量 (用于生成 data.yaml)")
    parser.add_argument("--names", nargs="+", help="类别名称列表 (如: cat dog person)")
    ...
    try:
        ...
        if args.classes:
            print(f"\n🎯 按类别筛选: {args.classes}")
            all_data = creator.filter_by_classes(all_data, target_classes=args.classes, include_all=args.include_all)
        ...
        if args.max_count:
            print(f"\n📊 数量限制: {args.max_count}")
            all_data = creator.sample_data(all_data, max_count=args.max_count, random_sample=args.random)
        ...
        if args.no_split:
            ...
            train_data, val_data = creator.split_dataset(
                all_data, val_ratio=args.val_ratio, random_split=not args.no_random_split
            )
        ...
        creator.create_summary(
            all_data=all_data, train_data=train_data, val_data=val_data, output_file=creator.output_dir / summary_file
        )
